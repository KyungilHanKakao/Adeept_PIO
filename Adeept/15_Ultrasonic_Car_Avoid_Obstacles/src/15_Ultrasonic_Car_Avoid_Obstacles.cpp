...
float leftDist;
float rightDist;
int avoid_Dist = 35; // cm
int minDist = 15;    // cm

int wheel_Steering = 35;
int deviation = -20;
int servo_Init = 90;
int Speed = 50;          // value:0-100
int steering_Speed = 50; // value:0-100
...
float angle0 = -6.5; // Mechanical balance angle 5.5
int slong;
...
void countpluse()
{
  ...
  { // Car movement direction to determine: back when (PWM is the motor voltage is negative) pulse number is negative
    rpluse = -rpluse;
    lpluse = -lpluse;
  }
  else if ((balancecar.pwm1 > 0) && (balancecar.pwm2 > 0))
  { // Car movement direction to determine: forward (PWM is the motor voltage is positive) pulse number is negative
    rpluse = rpluse;
    lpluse = lpluse;
  }
  else if ((balancecar.pwm1 < 0) && (balancecar.pwm2 > 0))
  { ////Car movement direction to determine: right rotation, the right pulse number is positive, the number of left pulse is negative.
    rpluse = rpluse;
    lpluse = -lpluse;
  }
  else if ((balancecar.pwm1 > 0) && (balancecar.pwm2 < 0))
  { // Car movement direction to determine: left rotation, the right pulse number is negative, the number of left pulse is positive.
    rpluse = -rpluse;
    lpluse = lpluse;
  }
  // To judge
...
void inter()
{
  ...
  speedcc++;
  if (speedcc >= 8)
  { // 50ms into the speed loop control
  ...
  if (turncount > 2)
  { // 10ms into the rotation control
    Serial.println("turncount");
...
void Avoid_Obstacles()
{
  ...
  if (distance < 30)
  {
    back = 25; // control 2WD balance car backwards
    front = 0;
    turnl = 0;
    turnr = 0;
    spinl = 0;
  ...
  {
    front = -25; // control 2WD balance car forwards
    back = 0;
    turnl = 0;
    turnr = 0;
    spinl = 0;
...
float getDistance()
{
  ...
  if (pingTime != 0)
  {                                                  // if the measure is not overtime
